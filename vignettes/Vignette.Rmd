...
The function `split.exprs()` splits the expression data of all proteins into individual protein complexes.
```{r,warning=F,message=F}
tumor_comp = split.exprs(tumor,ref)
normal_comp = split.exprs(normal,ref)
```
...
```{r,include=F,warning=F,message=F,fig.align="center",out.height="50%"}
...
```{r,message=F,warning=F,out.width="80%",fig.height=8,fig.width=12}
exprs = rbind(x1,x2)
